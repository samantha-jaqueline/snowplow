  ...
  module EmrEtlRunner
    class EmrJob
      ...
      def initialize(debug, staging, enrich, staging_stream_enrich, shred, es, archive_raw, rdb_load, archive_enriched, archive_shredded, config, enrichments_array, resolver, targets, rdbloader_steps)
        ...
        ami_version = Gem.version(config[:aws][:emr][:ami_version])
        ...
        if staging_stream_enrich
          ...
          unless Sluice::Storage::S3::is_empty?(s3, csbe_good_loc)
            raise DirectoryNotEmptyError, "Cannot safely add staging step to jobflow, #{csbe_good_loc} is not empty"
          end
    ...
    private
      ...
      def get_processing_manifest(targets)
        targets[:ENRICHED_EVENTS].select { |t| not t.dig(:data, :processingManifest).nil? }.map { |t| 
          t.dig(:data, :processingManifest)
        }.first
      end
